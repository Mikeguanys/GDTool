...
namespace GDataBase
{
    ...
    /// SQLROM
    /// </summary>
    public static class SQLServerModelBS
    {
        /// <summary>
        /// 数据库连接类
        /// </summary>
        ...
        /// <returns></returns>
        private static SQLServerDBHeper GetDBHeper(string Connection)
        {
            if (string.IsNullOrEmpty(Connection))
            {
                return new SQLServerDBHeper();
            }
            else
            {
                return new SQLServerDBHeper(Connection);
            }
        }
        /// <summary>
        /// 泛型查询默认主键排倒叙
        ...
        public static GDPaging<T> GTPaging<T>(this T Model, Expression<Func<T, bool>> Exp = null, int ThisPage = 1, int PageSize = 10, string Storing = null, string ConnectionName = null) where T : new()
        {
            ...
            SqlParameter[] parameter =
            {
               ...
               new SqlParameter("@PageSize",PageSize),
               new SqlParameter("@TableName",ModelName),
               new SqlParameter("@Condition",string.IsNullOrEmpty(Condition)?"":" and "+Condition),
               new SqlParameter("@Sorting",(string.IsNullOrEmpty(Storing)?(" Id desc"):Storing))
            };
            DataSet ds = Helper.MoreExecuteStorage("GT_Paging", parameter);
            if (ds == null || ds.Tables.Count < 2 || ds.Tables[0].Rows.Count == 0)
            ...
            foreach (PropertyInfo pi in propertys)
            {
                ...
                if (objAttrs != null)
                {
                    ...
                    if (obj != null)
                    {
                        FileName.Add(pi.Name);
                        Value.Add("@" + pi.Name);
                        ListParameter.Add(new SqlParameter("@" + pi.Name, pi.GetValue(Model, null)));
                    }
                }
            }
        ...
        public static int GTReturnAddId<T>(this T Model, string ConnectionName = null) where T : new()
        {
            ...
            SQLServerDBHeper Helper = GetDBHeper(ConnectionName);
            return Helper.GetAddGetId(string.Join(",", FileName), string.Join(",", Value), ModelName, Parameter.ToArray());
        }
        /// <summary>
        /// 直接用SQL查询返回一个表
        /// </summary>
        ...
        public static List<List<object>> GTMergerSearch(params object[] obj)
        {
            ...
            SQLServerDBHeper Helper = GetDBHeper(null);
            var r = Helper.BsDataSet(string.Join(";", ListSQLStr.Select(s => s.SQLStr).ToArray()), ListSQLStr.Select(s => s.EntityName).ToArray());
