...
namespace GDateBase2v.MySQL
{
    class MySQLBSHelper : MySQLBasis, SQLGDMethodInterface
    {
        ...
        /// 
        /// </summary>
        public MySQLBSHelper() { }
        /// <summary>
        /// 
        /// </summary>
        /// <param name="ConnectinName">WebConfig连接名字</param>
        public MySQLBSHelper(string ConnectinName) : base(ConnectinName) { }
        /// <summary>
        /// 添加并返回自增Id
        /// </summary>
        ...
        public int GDAddGetId<T>(T Model) where T : new()
        {
            ...
            return BsAddGetId($"insert into {typeof(T).Name}({string.Join(",", GetColum.Select(s => s.Field).ToList())}) VALUES({string.Join(",", GetColum.Select(s => "@" + s.Field).ToList())})", GetColum.Select(s => s.Parameter as MySqlParameter).ToArray());
        ...
        public bool GDAdd<T>(T Model) where T : new()
        {
            ...
            var GetColum = ListParameter.Where(w => !w.IsIncrease).Select(s => new { s.Field, s.Parameter }).ToList();
            return BsExecute($"insert into {typeof(T).Name}({string.Join(",", GetColum.Select(s => s.Field).ToList())}) VALUES({string.Join(",", GetColum.Select(s => "@" + s.Field).ToList())})", GetColum.Select(s => s.Parameter as MySqlParameter).ToArray());
        }
        /// <summary>
        /// 批量添加
        /// </summary>
        ...
        public bool GDAddBatch<T>(List<T> ListModel) where T : new()
        {
            ...
            List<MySqlParameter> ListObject = new List<MySqlParameter>();
            ...
            foreach (var item in ListModel)
            {
                ...
                for (int i = 0; i < valuestr.Length; i++)
                {
                    ...
                    ListObject.Add(ListParameter[i].Parameter as MySqlParameter);
                }
        ...
        public bool GDUpdate<T>(T Model, Expression<Func<T, bool>> Exp = null) where T : new()
        {
            ...
            return BsExecute($"UPDATE {typeof(T).Name} SET {string.Join(",", GetColum.Select(s => s.Field).ToList())} where 1=1 {Where}", GetColum.Select(s => s.Parameter as MySqlParameter).ToArray());
        }
        /// <summary>
        /// 根据Key更新
        /// </summary>
        ...
        public bool GDUpdateByKey<T>(T Model) where T : new()
        {
            ...
            var GetColum = ListParameter.Where(w => !w.IsKey).Select(s => new { Field = (s.Field + "=" + s.ParameterName), Parameter = s.Parameter }).ToList();
            return BsExecute($"UPDATE {typeof(T).Name} SET {string.Join(",", GetColum.Select(s => s.Field).ToList())} where {Keys.Field + "=" + Keys.Value}", GetColum.Select(s => s.Parameter as MySqlParameter).ToArray());
        }
        /// <summary>
        /// 删除操作
        /// </summary>
        ...
        public GDPaging<T> GDPaging<T>(IEnumerable<T> Source, int ThisPage = 1, int PageSize = 10) where T : new()
        {
            ...
            {
               new MySqlParameter("@ThisPage",ThisPage),
               new MySqlParameter("@PageSize",PageSize),
               new MySqlParameter("@TableName",typeof(T).Name),
               new MySqlParameter("@Condition",Where),
               new MySqlParameter("@Sorting",Sort)
            };
        ...
        public GDPaging<T> GDPaging<T>(T Source, Expression<Func<T, bool>> Exp, int ThisPage = 1, int PageSize = 10) where T : new()
        {
            ...
            MySqlParameter[] parameter =
            {
               ...
               new MySqlParameter("@PageSize",PageSize),
               new MySqlParameter("@TableName",typeof(T).Name),
               new MySqlParameter("@Condition",string.IsNullOrEmpty(Where)?"":(" and "+Where)),
               new MySqlParameter("@Sorting",Sort)
            };
            DataSet ds = BsStorageDs("GT_Paging", parameter);
            if (ds == null || ds.Tables.Count < 2 || ds.Tables[0].Rows.Count == 0)
        ...
        public GDPaging<T> GDPaging<T, TRuslt>(IEnumerable<T> Source, Expression<Func<T, TRuslt>> OrderBy, int ThisPage = 1, int PageSize = 10) where T : new()
        {
            ...
            MySqlParameter[] parameter =
            {
               ...
               new MySqlParameter("@TableName",typeof(T).Name),
               new MySqlParameter("@Condition",Where),
               new MySqlParameter("@Sorting",string.Join(",",Sort))
            };
            DataSet ds = BsStorageDs("GT_Paging", parameter);
            if (ds == null || ds.Tables.Count < 2 || ds.Tables[0].Rows.Count == 0)
            {
