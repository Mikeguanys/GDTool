...
namespace GDateBase2v.SQLite
{
    class SQLiteBSHelper : SQLiteBasis, SQLGDMethodInterface
    {
        ...
        /// 
        /// </summary>
        public SQLiteBSHelper() { }
        /// <summary>
        /// 
        /// </summary>
        /// <param name="ConnectinName">WebConfig连接名字</param>
        public SQLiteBSHelper(string ConnectinName) : base(ConnectinName) { }
        /// <summary>
        /// 添加并返回自增Id
        /// </summary>
        ...
        public int GDAddGetId<T>(T Model) where T : new()
        {
            ...
            return BsAddGetId($"insert into {typeof(T).Name}({string.Join(",", GetColum.Select(s => s.Field).ToList())}) VALUES({string.Join(",", GetColum.Select(s => "@" + s.Field).ToList())})", GetColum.Select(s => s.Parameter as SQLiteParameter).ToArray());
        ...
        public bool GDAdd<T>(T Model) where T : new()
        {
            ...
            var GetColum = ListParameter.Where(w => !w.IsIncrease).Select(s => new { s.Field, s.Parameter }).ToList();
            return BsExecute($"insert into {typeof(T).Name}({string.Join(",", GetColum.Select(s => s.Field).ToList())}) VALUES({string.Join(",", GetColum.Select(s => "@" + s.Field).ToList())})", GetColum.Select(s => s.Parameter as SQLiteParameter).ToArray());
        }
        /// <summary>
        /// 批量添加
        /// </summary>
        ...
        public bool GDAddBatch<T>(List<T> ListModel) where T : new()
        {
            ...
            List<SQLiteParameter> ListObject = new List<SQLiteParameter>();
            ...
            foreach (var item in ListModel)
            {
                ...
                for (int i = 0; i < valuestr.Length; i++)
                {
                    ...
                    ListObject.Add(ListParameter[i].Parameter as SQLiteParameter);
                }
        ...
        public bool GDUpdateByKey<T>(T Model) where T : new()
        {
            ...
            var GetColum = ListParameter.Where(w => !w.IsKey).Select(s => new { Field = (s.Field + "=" + s.ParameterName), Parameter = s.Parameter }).ToList();
            return BsExecute($"UPDATE {typeof(T).Name} SET {string.Join(",", GetColum.Select(s => s.Field).ToList())} where {Keys.Field + "=" + Keys.Value}", GetColum.Select(s => s.Parameter as SQLiteParameter).ToArray());
        }
        /// <summary>
        /// 删除操作
        /// </summary>
        ...
        public GDPaging<T> GDPaging<T>(IEnumerable<T> Source, int ThisPage = 1, int PageSize = 10) where T : new()
        {
            ...
            SQLiteParameter[] parameter =
            {
               ...
               new SQLiteParameter("@TableName",typeof(T).Name),
               new SQLiteParameter("@Condition",Where),
               new SQLiteParameter("@Sorting",Sort)
            };
        ...
        public GDPaging<T> GDPaging<T>(T Source, Expression<Func<T, bool>> Exp, int ThisPage = 1, int PageSize = 10) where T : new()
        {
            ...
            SQLiteParameter[] parameter =
            {
               ...
               new SQLiteParameter("@PageSize",PageSize),
               new SQLiteParameter("@TableName",typeof(T).Name),
               new SQLiteParameter("@Condition",string.IsNullOrEmpty(Where)?"":(" and "+Where)),
               new SQLiteParameter("@Sorting",Sort)
            };
            DataSet ds = BsStorageDs("GT_Paging", parameter);
            if (ds == null || ds.Tables.Count < 2 || ds.Tables[0].Rows.Count == 0)
        ...
        public GDPaging<T> GDPaging<T, TRuslt>(IEnumerable<T> Source, Expression<Func<T, TRuslt>> OrderBy, int ThisPage = 1, int PageSize = 10) where T : new()
        {
            ...
            SQLiteParameter[] parameter =
            {
               ...
               new SQLiteParameter("@TableName",typeof(T).Name),
               new SQLiteParameter("@Condition",Where),
               new SQLiteParameter("@Sorting",string.Join(",",Sort))
            };
            DataSet ds = BsStorageDs("GT_Paging", parameter);
            if (ds == null || ds.Tables.Count < 2 || ds.Tables[0].Rows.Count == 0)
            {
